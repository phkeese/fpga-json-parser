...
static void count_string_chars(simdjson_result<ondemand::value> value, uint64_t &count);
static void count_string_chars(simdjson_result<ondemand::array> array, uint64_t &count);
static void count_string_chars(simdjson_result<ondemand::object> object, uint64_t &count);

static void count_string_chars(simdjson_result<ondemand::value> value, uint64_t &count) {
	switch (value.type()) {
	case json_type::array:
		count_string_chars(value.get_array(), count);
		break;
	case json_type::object:
		count_string_chars(value.get_object(), count);
		break;
	case json_type::string:
		for (char c : value.get_string().value()) {
			count += c;
		}
		// char c = value.get_string().value().data()[0];
...
static void count_string_chars(simdjson_result<ondemand::array> array, uint64_t &count) {
	for (auto element : array) {
		count_string_chars(element, count);
	}
}

static void count_string_chars(simdjson_result<ondemand::object> object, uint64_t &count) {
	for (auto field : object) {
		for (char c : field.unescaped_key().value()) {
			count += c;
		}
		// count += field.unescaped_key().value().size();
		count_string_chars(field.value(), count);
	}
}

static void COUNT_STRING_CHARS_SIMDJSON(benchmark::State &state, const std::string &filename) {
	const auto json = simdjson::padded_string::load(filename);
	for (auto _ : state) {
		auto parser = simdjson::ondemand::parser{};
		auto document = parser.iterate(json);
		auto value = document.get_value();
		uint64_t count = 0;
		count_string_chars(value, count);
		(void)count;
		// std::cout << count << std::endl;
	}
}
...
void register_simdjson_benchmarks_for(const std::string &dirname, const std::string &filename) {
	...
	benchmark::RegisterBenchmark("simdjson::string_lengths::" + filename, COUNT_STRING_LENGTHS_SIMDJSON,
								 dirname + filename);
	benchmark::RegisterBenchmark("simdjson::string_chars::" + filename, COUNT_STRING_CHARS_SIMDJSON,
								 dirname + filename);
}