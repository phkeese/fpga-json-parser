...
//
template <typename T, typename, T begin, bool increase> struct integer_range_impl;

// incrementing case
template <typename T, T... N, T begin> struct integer_range_impl<T, std::integer_sequence<T, N...>, begin, true> {
	using type = std::integer_sequence<T, N + begin...>;
};

// decrementing case
template <typename T, T... N, T begin> struct integer_range_impl<T, std::integer_sequence<T, N...>, begin, false> {
...
using integer_range =
	typename integer_range_impl<T, std::make_integer_sequence<T, (begin < end) ? end - begin : begin - end>, begin,
								(begin < end)>::type;
...
//
template <class T, T begin, T end> using make_integer_range = integer_range<T, begin, end>;
...
//
template <std::size_t begin, std::size_t end> using make_index_range = integer_range<std::size_t, begin, end>;
...
template <typename T, T... Pows> struct integer_pow2_sequence_impl<T, std::integer_sequence<T, Pows...>> {
	using type = std::integer_sequence<T, (T(1) << Pows)...>;
};

// integer_pow2_sequence
template <typename T, T N>
using integer_pow2_sequence = typename integer_pow2_sequence_impl<T, std::make_integer_sequence<T, N>>::type;
...
//
template <class T, T N> using make_integer_pow2_sequence = integer_pow2_sequence<T, N>;
...
//
template <std::size_t N> using make_index_pow2_sequence = integer_pow2_sequence<std::size_t, N>;
...
template <class T, typename = void> struct has_subscript_impl : std::false_type {};

template <typename T> struct has_subscript_impl<T, void_t<decltype(std::declval<T>()[1])>> : std::true_type {};
} // namespace detail

template <typename T> struct has_subscript {
	static constexpr bool value = std::is_same_v<typename detail::has_subscript_impl<T>::type, std::true_type>;
};

template <typename T> inline constexpr bool has_subscript_v = has_subscript<T>::value;
...
template <typename T> inline constexpr bool is_sycl_pipe_v = is_sycl_pipe<T>::value;
